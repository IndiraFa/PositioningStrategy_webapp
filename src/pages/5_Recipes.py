...
def display_header():
    ...
    st.write(
       ...
       main cooking method of the recipes, e.g 'vegetarian', 'dinner', 'course'
       (see the word cloud below).\n 
       We plan to explore the information from these labels that is considered 
       useful for health. 
       We have associated each recipe with a rating and a label, 
       the study will sort the recipes with the selected labels, 
       study the distribution of nutriscores, 
       the highest nutriscores and then focus on the distribution of 
       nutrients according to the label.
       
       Next to the dietary keywords, there are also typical cooking labels 
       like 'preparation', 'main ingredient', 'main dish'. 
       Try to find your 'dinner' or 'beginner cook' if you want 
    ...
    st.markdown("""
        <div class="container_with_border">
            <h3>💡 Key takeaways</h3>
            <p>The labels are clearly consistent with our nutriscore calculated
                 on page 2. 
            "Low protein" recipes cannot offer a good ratio with nutrients and 
                are therefore rated C or D. 
...
def display_boxplot(all_recipes, tags_input):
    """
    Display the boxplot of the nutriscore & 
    label of all recipes having the tags
    """
