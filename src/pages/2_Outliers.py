...
def display_introduction():
    ...
    st.markdown(
        ...
        In this page the purpose is to explore a set of nutritional 
        data extracted from a PostgreSQL database. Through this 
        interface, you can visualize data, understand processing 
        steps, and identify outliers.

        Take your time to explore each step to better understand 
...
def load_and_explore_raw_data(raw_data):
    ...
    st.write(
        """
        Data from the nutrition column are first extracted from the 
        table following this schema:\n
...
def analyze_formatted_data(formatted_data, normalized_data):
    ...
    """
    logger.debug(
        "Display: Analyzing formatted data and normalized data."
    )
    ...
    st.markdown(
        """
        Our data refers to 1 portion and has been adjusted, in 
        percentage terms, to daily values of 2000 calories for an adult.
        """
    )
    st.latex(
        r"""
    ...
    logger.debug(
        "Display: Analyzing formatted data and normalized data successfully."
    )
...
def identify_outliers_with_manual_filters():
    ...
    st.markdown(
        ...
        steps: \n
        - <u>Step 1: Manual filtering by applying thresholds beyond 
          which outliers are identified.</u> \n   
        """,
        unsafe_allow_html=True,
    )
    ...
    logger.debug(
        "Display: Outliers identified with manual filters successfully."
    )
...
def apply_z_score_method(outliers_size):
    ...
    st.markdown(
        """
        - <u>Step 2: Applying the Z-score method to identify outliers.</u> \n
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.expander("Note: Click to display more information"):
        st.write(
            f"""
            Number of recipes removed after applying thresholds followed by 
            the Z-score method = {outliers_size}
            """
        )
...
def visualize_data_distribution(
    ...
    selected_option = st.selectbox(
        'Data visualization:', list(options.keys())
    )
    ...
    with col1:
        ...
        fig = px.histogram(
            filtered_data, x=y_column, title=selected_option
        )
        st.plotly_chart(fig)
    ...
    with col2:
        ...
        fig_box = px.box(
            filtered_data, y=y_column, title=f"Box Plot - {selected_option}"
        )
    ...
    logger.debug(
        "Graphical visualization of data distributions displayed successfully."
    )
...
def display_conclusion():
    ...
    st.markdown(
        ...
        <div class="container_with_border">
            <h3>💡 Key takeaways</h3>
            <p>
                The interactive visualization allows for better understanding 
                of distributions of various nutritional values across different 
                categories of food recipes. The data cleaning steps, including 
                manual filtering and Z-score method, ensure better quality of 
...
def main():
    ...
    logger.debug("Retrieve data from the database")
    ...
    logger.debug("Extracting data from the dictionary.")
    ...
    visualize_data_distribution(
        normalized_data, prefiltre_data, nutrition_noOutliers
    )
