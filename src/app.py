...
from utils.data_loader import load_datafiles
...
def main():
    ...
    load_datafiles()
    path = "datasets/nutrition_data.db"
    conn = sqlite3.connect(path,)

    data_with_outliers_query = "SELECT * FROM nutrition_withOutliers"
    data_no_outliers_query = "SELECT * FROM nutrition_noOutliers"

    display_header()

    data_with_outliers = pd.read_sql_query(data_with_outliers_query, conn)
    data_no_outliers = pd.read_sql_query(data_no_outliers_query, conn)
