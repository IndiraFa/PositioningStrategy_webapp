...
def download_data(url, output_dir="src/datasets"):
    ...
    try:
        ...
        if response.status_code == 200:
            # Extract filename and construct full file path
            ...
            logger.error(
                f"Error while downloading, status: {response.status_code}"
            )
            return None
...
def load_datafiles():
    """
    Loads data from multiple URLs specified in environment variables
    ...
    if not urls:
        logger.error("""
            No download links provided in the environment variables.
        """)
        return
    ...
    for url in urls:
        ...
        if not validate_url(url):
            logger.info("lien téléchargé", url)
            continue
