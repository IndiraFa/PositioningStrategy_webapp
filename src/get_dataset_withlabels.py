...
def calcul_nutriscore_with_outliers(data, grille, configs):
    ...
    nutriscore_calculator = NutriScore(processed_data, grille, configs)
    
    # Appeler la méthode calcul_nutriscore sur l'instance de NutriScoreCalculator
    nutriscore_with_outliers_nolabel= nutriscore_calculator.calcul_nutriscore()
    nutriscore_with_outliers = nutriscore_calculator.set_scorelabel()
...
df_nutriscore_with_outliers = calcul_nutriscore_with_outliers(path, path_grille, configs)
print(df_nutriscore_with_outliers)
print(df_nutriscore_with_outliers.shape)
