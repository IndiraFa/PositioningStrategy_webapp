...
from utils.data_loader import validate_url, download_data, load_datafiles
...
def test_validate_url_empty(mock_logger):
    ...
    mock_logger.error.assert_called_once_with(
        "The download link is not defined in the .env file."
    )
...
def test_validate_url_invalid(mock_logger):
    ...
    mock_logger.error.assert_called_once_with(
        "The download link is not a valid URL."
    )
...
def test_download_data_success(mock_logger, mock_get):
    """Test download_data with a successful HTTP request."""
    # Mock a successful HTTP request response
    ...
    with patch("builtins.open", new_callable=MagicMock) as mock_open:
        ...
        # Assertions for file path, requests, and write calls
        expected_file_path = os.path.join(output_dir, "fakefile.txt")
        ...
        # Logger message assertion
        mock_logger.info.assert_called_with(
            f"Download completed successfully. File saved as {file_path}"
        )
...
def test_download_data_failure(mock_logger, mock_get):
    """Test download_data when the HTTP request fails with a 404 status."""
    # Mock an HTTP response with status code 404
    mock_response = MagicMock()
    ...
    mock_get.assert_called_once_with(url)
    mock_logger.error.assert_called_once_with(
        "Error while downloading, status: 404"
    )
...
def test_load_data(mock_getenv, mock_validate_url, mock_download_data):
    ...
    mock_getenv.side_effect = lambda var_name, default=None: (
        ...
        if var_name == "DRIVE_LINKS"
        else "test_datasets"
    )
    mock_validate_url.side_effect = [True, True]
    mock_download_data.side_effect = [
        os.path.join("test_datasets", "file1.txt"),
        os.path.join("test_datasets", "file2.txt")
    ]
    ...
    mock_download_data.assert_any_call(
        "http://example.com/file1.txt", "test_datasets"
    )
    mock_download_data.assert_any_call(
        "http://example.com/file2.txt", "test_datasets"
    )
